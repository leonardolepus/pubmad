...
def KGML2Graph(xmlfile, filter_by = ()):
    """
    Parse a KGML file and return a PyNx graph object
    ...
    KGML2Graph return a KeggPathway object, derived from nx.LabeledGraph:
    >>> print type(pathway)
    <class 'KeggPathway.KeggPathway'>

    See help(KGMLGraph) for more docs.
    You can refer to help(nx.LabeledGraph) and http://nx.lanl.gov/reference/classes.labeledgraph.html
...
#        if node_type in entriestype:       # something else?
        names = [int(gene.split(':')[1]) for gene in entry.get('name').split(' ')]
...
#            node_title = node_title[:-3]
        ...
        nodes[node_id] = names#(names, node_title, node_type)
        #pathway.labels[node_id] = node_title
        for name in names:
            pathway.add_node(name)#, data={'label': node_title, 'type': node_type})
...
def plot_original(pathway):
    ...
    pylab.figure()
    nx.draw_nx(pathway, pos, labels=pathway.labels)
    title = pathway.title.replace('/', '-') # TODO: which is the proper way to remove / in a filename?
...
def convert_to_gml(pathway):
    ...
    """
    nx.write_gml(pathway, pathway.title + '.gml')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pathwayfile', '--pathway', dest='pathwayfile', type=str, default='hsa04151.xml')
